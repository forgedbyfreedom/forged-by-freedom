...
Replaces the old ingest_to_wordpress.py, which POSTed to the WordPress
/fbf/v1/rag/upsert endpoint — the WRONG store (writing there fills
wp_fbf_rag_chunks, whose PHP full-table cosine scan can take the site down)
and which returned HTTP 520 on every run, so the backlog never drained.

This targets the same endpoint the working PC pipeline uses:
    POST {FBF_AI_URL}/ingest
    header  X-API-Key: {FBF_AI_API_KEY}
    body    {"source": <relpath>, "text": <file contents>}
The server chunks + embeds server-side with its own OpenAI key and upserts into
Qdrant (collection fbf_pinecone). It is idempotent on deterministic IDs, so
re-sending the same source+text never duplicates points — safe to run alongside
the PC nightly.

No OpenAI key is needed here: the VPS does the embedding.
"""
...
MAX_FILES  = int(os.environ.get("MAX_CHUNKS_PER_RUN", "0") or "0")   # 0 = no cap

# Where transcripts live in the checkout. First existing root(s) are scanned.
...
def main():
    ...
    # Preflight — cheap, unauthenticated, tells us the tunnel/service is up.
    try:
    ...
    for path in files:
        ...
        if not text:
            state[rel] = h  # record empty files so we don't re-check them
            continue
        ...
        if ok:
            ...
            # Never crash the whole run on one file — keep draining the backlog.
    ...
    # Green build on success/partial (so failure emails stop). Red only when the
    # endpoint is clearly broken: nothing sent, nothing skipped, and failures seen
    # (auth wrong, service/tunnel down, or endpoint changed).

...
function getIdGenerator(bytes2) {
  return function generateId() {
    for (var i = 0; i < bytes2 / 4; i++) {
    ...
    for (var i = 0; i < bytes2; i++) {
      if (SHARED_BUFFER[i] > 0) {
        ...
        SHARED_BUFFER[bytes2 - 1] = 1;
      }
    }
    return SHARED_BUFFER.toString("hex", 0, bytes2);
  };
}
...
function hrTimeDuration(startTime, endTime) {
  var seconds2 = endTime[0] - startTime[0];
...
var init_timeout = __esm({
  "../../node_modules/@opentelemetry/core/build/esm/utils/timeout.js"() {
    ...
    TimeoutError2 = /** @class */
    function(_super) {
      __extends(TimeoutError3, _super);
...
var require_utils = __commonJS({
  "../../node_modules/@opentelemetry/instrumentation-http/build/src/utils.js"(exports) {
    ...
    var satisfiesPattern = (constant2, pattern) => {
      if (typeof pattern === "string") {
        return pattern === constant2;
      } else if (pattern instanceof RegExp) {
        return pattern.test(constant2);
    ...
    var isIgnored = (constant2, list, onException) => {
      if (!list) {
        return false;
      }
      try {
        for (const pattern of list) {
          if ((0, exports.satisfiesPattern)(constant2, pattern)) {
            return true;
          }
        }
      } catch (e) {
  ...
  "../../node_modules/resolve/lib/normalize-options.js"(exports, module2) {
    ...
    module2.exports = function(_, opts) {
      ...
      const runPnpResolution = (request, basedir) => {
        ...
        const packagePath = path4.dirname(manifestPath);
        const unqualifiedPath = typeof parts[2] !== `undefined` ? path4.join(packagePath, parts[2]) : packagePath;
      ...
      const paths = (request, basedir, getNodeModulePaths, opts2) => {
        ...
        if (request.match(/^@[^/]+\//))
          nodeModules = path4.dirname(nodeModules);
        return [nodeModules];
      };
...
var require_async = __commonJS({
  "../../node_modules/resolve/lib/async.js"(exports, module2) {
    ...
    module2.exports = function resolve2(x, options, callback) {
      ...
      var readFile2 = opts.readFile || fs5.readFile;
...
var require_sync = __commonJS({
  "../../node_modules/resolve/lib/sync.js"(exports, module2) {
    ...
    module2.exports = function resolveSync(x, options) {
      ...
      var isFile = opts.isFile || defaultIsFile;
      var readFileSync3 = opts.readFileSync || fs5.readFileSync;
      var isDirectory = opts.isDirectory || defaultIsDir;
...
var require_module_details_from_path = __commonJS({
  "../../node_modules/module-details-from-path/index.js"(exports, module2) {
    ...
    module2.exports = function(file) {
      var segments = file.split(path4.sep);
      ...
      return {
        name,
        basedir: segments.slice(0, index + offset).join(path4.sep),
        path: segments.slice(index + offset).join(path4.sep)
      };
    };
  }
});
...
var require_require_in_the_middle = __commonJS({
  "../../node_modules/require-in-the-middle/index.js"(exports, module2) {
    ...
    function Hook2(modules, options, onrequire) {
      ...
      function patchedRequire(args, coreOnly) {
        ...
        if (core === true) {
          ...
          const parsedPath = path4.parse(filename);
          ...
          if (!matchFound) {
            ...
            if (res !== filename) {
              if (internals === true) {
                moduleName = moduleName + path4.sep + path4.relative(basedir, filename);
    ...
    function resolveModuleName(stat) {
      const normalizedPath = path4.sep !== "/" ? stat.path.split(path4.sep).join("/") : stat.path;
      return path4.posix.join(stat.name, normalizedPath).replace(normalize3, "");
    }
  }
});
        ...
        if (modules) {
          for (const moduleName of modules) {
            if (moduleName === name) {
              if (baseDir) {
                if (internals) {
                  name = name + path4.sep + path4.relative(baseDir, fileURLToPath2(filename));
                } else {
  ...
  "../../node_modules/@opentelemetry/instrumentation/build/esm/platform/node/instrumentation.js"() {
    ...
    function(_super) {
      ...
      InstrumentationBase2.prototype._extractPackageVersion = function(baseDir) {
        try {
          var json = readFileSync(path3.join(baseDir, "package.json"), {
            encoding: "utf8"
          });
      ...
      InstrumentationBase2.prototype._onRequire = function(module2, exports, name, baseDir) {
        ...
        var normalizedName = path3.normalize(name);
      ...
      InstrumentationBase2.prototype.enable = function() {
        ...
        var _loop_1 = function(module_22) {
          ...
          var hook = path3.isAbsolute(module_22.name) ? new import_require_in_the_middle2.Hook([module_22.name], { internals: true }, onRequire) : this_1._requireInTheMiddleSingleton.register(module_22.name, onRequire);
  ...
  "../../node_modules/@opentelemetry/instrumentation-http/build/src/http.js"(exports) {
    ...
    var HttpInstrumentation2 = class extends instrumentation_1.InstrumentationBase {
      ...
      _closeHttpSpan(span, spanKind, startTime, metricAttributes) {
        ...
        const duration2 = (0, core_1.hrTimeToMilliseconds)((0, core_1.hrTimeDuration)(startTime, (0, core_1.hrTime)()));
        if (spanKind === api_1.SpanKind.SERVER) {
          this._httpServerDurationHistogram.record(duration2, metricAttributes);
        } else if (spanKind === api_1.SpanKind.CLIENT) {
          this._httpClientDurationHistogram.record(duration2, metricAttributes);
        }
      }
      ...
      _applyPatch(layer, layerPath) {
        ...
        this._wrap(layer, "handle", (original) => {
          ...
          const patched = function(req, res) {
            ...
            const route = req[internal_types_1._LAYERS_STORE_PROPERTY].filter((path4) => path4 !== "/" && path4 !== "/*").join("").replace(/\/{2,}/g, "/");
            const attributes = {
...
var require_utils4 = __commonJS({
  "../../node_modules/@opentelemetry/instrumentation-graphql/build/src/utils.js"(exports) {
    ...
    function createFieldIfNotExists(tracer, getConfig, contextValue, info, path4) {
      let field = getField(contextValue, path4);
      let spanAdded = false;
      if (!field) {
        spanAdded = true;
        const parent = getParentField(contextValue, path4);
        field = {
          parent,
          span: createResolverSpan(tracer, getConfig, contextValue, info, path4, parent.span),
          error: null
        };
        addField(contextValue, path4, field);
      }
      return { spanAdded, field };
    }
    function createResolverSpan(tracer, getConfig, contextValue, info, path4, parentSpan) {
      var _a2, _b;
      const attributes = {
        [AttributeNames_1.AttributeNames.FIELD_NAME]: info.fieldName,
        [AttributeNames_1.AttributeNames.FIELD_PATH]: path4.join("."),
    ...
    function addField(contextValue, path4, field) {
      return contextValue[symbols_1.OTEL_GRAPHQL_DATA_SYMBOL].fields[path4.join(".")] = field;
    }
    function getField(contextValue, path4) {
      return contextValue[symbols_1.OTEL_GRAPHQL_DATA_SYMBOL].fields[path4.join(".")];
    }
    function getParentField(contextValue, path4) {
      for (let i = path4.length - 1; i > 0; i--) {
        const field = getField(contextValue, path4.slice(0, i));
        if (field) {
          return field;
        }
      }
    ...
    function pathToArray(mergeItems, path4) {
      const flattened = [];
      let curr = path4;
      while (curr) {
        let key = curr.key;
    ...
    function wrapFields(type, tracer, getConfig) {
      ...
      const fields6 = type.getFields();
      type[symbols_1.OTEL_PATCHED_SYMBOL] = true;
      Object.keys(fields6).forEach((key) => {
        const field = fields6[key];
        if (!field) {
          return;
        }
        if (field.resolve) {
    ...
    function wrapFieldResolver(tracer, getConfig, fieldResolver, isDefaultResolver = false) {
      ...
      function wrappedFieldResolver(source, args, contextValue, info) {
        ...
        const path4 = pathToArray(config.mergeItems, info && info.path);
        const depth = path4.filter((item) => typeof item === "string").length;
        let field;
        let shouldEndSpan = false;
        if (config.depth >= 0 && config.depth < depth) {
          field = getParentField(contextValue, path4);
        } else {
          const newField = createFieldIfNotExists(tracer, getConfig, contextValue, info, path4);
    ...
    var MySQLInstrumentation2 = class _MySQLInstrumentation extends instrumentation_1.InstrumentationBase {
      ...
      _patchCallbackQuery(span, parentContext) {
        return (originalCallback) => {
          return function(err, results, fields6) {
            if (err) {
              span.setStatus({
                code: api_1.SpanStatusCode.ERROR,
                message: err.message
    ...
    var IORedisInstrumentation2 = class extends instrumentation_1.InstrumentationBase {
      ...
      _traceConnection(original) {
        ...
        return function() {
          ...
          try {
            const client2 = original.apply(this, arguments);
            (0, utils_1.endSpan)(span, null);
            return client2;
          } catch (error) {
            (0, utils_1.endSpan)(span, error);
      ...
      _patchRouterDispatch(dispatchLayer) {
        ...
        for (const pathLayer of routesStack) {
          ...
          for (let j = 0; j < pathStack.length; j++) {
            ...
            pathStack[j] = this._patchLayer(routedMiddleware, true, path4);
          }
        }
...
function flattenParams(data, path4 = "") {
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    const fieldPath = resolvePath2(path4, key);
    if (isObject2) {
...
function resolvePath2(path4, key) {
  return `${path4}${path4 ? "." : ""}${key}`;
}
function toURLSearchParams(params, isComplexRequest) {
  const flatten2 = flattenParams(params);
  const isPayloadNonSerializableAsUrlSearchParams = Object.entries(flatten2).some(([key, value]) => key.includes(".") || Array.isArray(value) && value.some((v) => typeof v === "object"));
  ...
  if (shouldSerializeToRParam) {
    ...
    return Object.entries(flatten2).reduce((urlSearchParams, [key, value]) => {
...
function getMyAccount(payload4) {
  var _a2 = serializer(_getMyAccountRequest, {}), toReq = _a2.toJSON, fromReq = _a2.fromJSON;
  var fromRes = serializer(_accountResponse, { _account }).fromJSON;
  function __getMyAccount(_a3) {
    var host = _a3.host;
...
function token(payload4) {
  var _a2 = serializer(_rawHttpRequest, {}), toReq = _a2.toJSON, fromReq = _a2.fromJSON;
  var fromRes = serializer(_rawHttpResponse, {}).fromJSON;
  function __token(_a3) {
    var host = _a3.host;
    var serializedData = toReq(payload4);
    var metadata = {
...
function deviceCode(payload4) {
  var _a2 = serializer(_deviceCodeRequest, {}), toReq = _a2.toJSON, fromReq = _a2.fromJSON;
  var fromRes = serializer(_deviceCodeResponse, {}).fromJSON;
  function __deviceCode(_a3) {
    var host = _a3.host;
    var serializedData = toReq(payload4);
    var metadata = {
      entityFqdn: "wix.identity.oauth.v1.refresh_token",
...
function revokeRefreshToken(payload4) {
  var _a2 = serializer(_revokeRefreshTokenRequest, {}), toReq = _a2.toJSON, fromReq = _a2.fromJSON;
  var fromRes = serializer(_revokeRefreshTokenResponse, {}).fromJSON;
  function __revokeRefreshToken(_a3) {
    var host = _a3.host;
    var serializedData = toReq(payload4);
    var metadata = {
      entityFqdn: "wix.identity.oauth.v1.refresh_token",
...
// ../cli-auth/src/useAuth.ts
function useAuth() {
  const context2 = (0, import_react2.useContext)(AuthContext);
  if (!context2) {
...
var BiContext = (0, import_react3.createContext)(null);
var BiProvider = function({
  children,
  value: biLogger
}) {
  return /* @__PURE__ */ import_react3.default.createElement(BiContext.Provider, { value: biLogger }, children);
};
var useBiLogger = () => {
  const context2 = (0, import_react3.useContext)(BiContext);
  if (context2 == null) {
...
${errorMessage}`
    ...
    AppProjectTypeNotImplemented: () => {
      return () => /* @__PURE__ */ import_react9.default.createElement(ErrorMessage, { message: "Currently 'App' project type is not supported." });
    }
  });
}
...
function getRequestUrl(requestOptions) {
  ...
  return `${protocol}//${hostname2}${port}${path4}`;
}
  ...
  function() {
    ...
    _httpInstrumentation = new _InstrumentationClass({
      ...
      requestHook: (span, req) => {
        ...
        if (client2 && client2.getOptions().autoSessionTracking) {
...
function getAbsoluteUrl(origin, path4 = "/") {
  const url = `${origin}`;
  if (origin.endsWith("/") && path4.startsWith("/")) {
    return `${url}${path4.slice(1)}`;
  }
  if (!origin.endsWith("/") && !path4.startsWith("/")) {
    return `${url}/${path4.slice(1)}`;
  }
...
function shouldSkipContextLinesForFile(path4) {
  if (path4.startsWith("node:")) return true;
  if (path4.endsWith(".min.js")) return true;
  if (path4.endsWith(".min.cjs")) return true;
  if (path4.endsWith(".min.mjs")) return true;
  if (path4.startsWith("data:")) return true;
  return false;
}
...
function getContextLinesFromFile(path4, ranges, output) {
  return new Promise((resolve2, _reject) => {
    ...
    function onStreamError(e) {
      ...
      DEBUG_BUILD && logger.error(`Failed to read file: ${path4}. Error: ${e}`);
      lineReaded.close();
...
var _localVariablesSyncIntegration = (options = {}, sessionOverride) => {
  ...
  return {
    ...
    setupOnce() {
      const client2 = getClient();
      const clientOptions = _optionalChain([client2, "optionalAccess", (_21) => _21.getOptions, "call", (_22) => _22()]);
        ...
        (session) => {
          ...
          if (captureAll) {
            ...
            rateLimiter = createRateLimiter(
              ...
              (seconds2) => {
                logger.log(
                  `Local variables rate-limit exceeded. Disabling capturing of caught exceptions for ${seconds2} seconds.`
                );
...
function createErrorHandler({
  ...
  const registerToProcessErrors = function() {
    process7.on("uncaughtException", (error) => {
      handler({
        error,
        data: { mechanism: { handled: false, type: "uncaughtException" } }
      });
    });
    process7.on("unhandledRejection", (error) => {
...
//# sourceMappingURL=chunk-6PYC5DBT.js.map
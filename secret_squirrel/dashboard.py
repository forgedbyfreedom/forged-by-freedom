...
<body>
  ...
  <div class="row">
    ...
    <div class="card">
      ...
      <p class="sub">Label the question, then start recording. Stops automatically
      after 1.5s of silence.</p>
  ...
  <div class="card" style="margin-top:16px;">
    <h3>History</h3>
    <table id="histTable">
      <thead><tr><th>#</th><th>Label</th><th>Stress</th><th>Level</th>
        <th>Duration</th><th>When</th></tr></thead>
      <tbody></tbody>
    </table>
  </div>
...
async function submitUrl(mode) {
  ...
  const r = await post('/api/analyze', {url, mode, label});
...
evt.onmessage = (e) => {
  ...
  (s.history || []).slice().reverse().forEach((h, idx) => {
    ...
    tr.innerHTML = `<td>${s.history_count - idx}</td><td>${h.label||''}</td><td>${c}</td><td>${lvl}</td><td>${(h.duration_sec||0).toFixed(1)}s</td><td>${new Date(h.timestamp*1000).toLocaleTimeString()}</td>`;
...
def api_question():
    body = request.get_json(silent=True) or {}
    return jsonify(engine.start_question(label=str(body.get("label", ""))))
